//! string_lang_lingua_all_languages_with_devanagari_script — enumerate compiled-in lingua Devanagari-script languages.
//!
//! Delegates to Language::all_with_devanagari_script() and emits one ISO 639-1 code per row.
...
fn evaluate_impl(_args: Vec<Value>) -> Result<BindingSets, String> {
        let mut langs: Vec<Language> = Language::all_with_devanagari_script().into_iter().collect();
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, _args: Vec<WitTerm>) -> Result<WitTerm, String> {
        Err(format!(
            "string_lang_lingua_all_languages_with_devanagari_script: unknown filter function '{name}' (use as a property function)"
        ))
    }
}
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "string_lang_lingua_all_languages_with_devanagari_script: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("string_lang_lingua_all_languages_with_devanagari_script: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    fn register_property_functions() -> Vec<PropertyDescriptor> {
        vec![PropertyDescriptor {
            name: "string_lang_lingua_all_languages_with_devanagari_script".to_string(),
            subject_arity: 0,
            object_arity: 0,
        }]
    }
    ...
    fn evaluate(
        ...
        match name.as_str() {
            "string_lang_lingua_all_languages_with_devanagari_script" => {
                let mut args = subjects;
                args.extend(objects);
                let bs = evaluate_impl(args)?;
                Ok(to_binding_rows(bs))
            }
            other => Err(format!("string_lang_lingua_all_languages_with_devanagari_script: unknown property function '{other}'")),
        }
    }
}
