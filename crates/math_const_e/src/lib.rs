//! math_const_e — mathematical constant e.
//!
//! Ported from the module-mode ABI (semantalytics/stardog-webfunctions/
//! function_math_constants/e) to the Component Model ABI.
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "math_const_e" => evaluate_impl(args),
            other => Err(format!("math_const_e: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
        if !args.is_empty() {
            return Err(format!("math_const_e: expected 0 args, got {}", args.len()));
        }
        Ok(decimal_literal(consts::E))
    }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "math_const_e: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("math_const_e: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "math_const_e: unknown property function '{name}' (this component provides none)"
        ))
    }
}
