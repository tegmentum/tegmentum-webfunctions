...
impl Guest for Component {
    fn evaluate(args: Vec<Value>) -> Result<BindingSets, String> {
        ...
        Ok(BindingSets {
            ...
            rows: vec![vec![Binding {
                name: "result".into(),
                value: decimal_literal(consts::TAU),
            }]],
        })
    }

    fn aggregate_step(_args: Vec<Value>, _mult: u64) -> Result<(), String> {
        Err("math_const_tau: aggregate-step not implemented".into())
    }

    fn aggregate_finish() -> Result<BindingSets, String> {
        Err("math_const_tau: aggregate-finish not implemented".into())
    }
    ...
    fn doc() -> BindingSets {
        BindingSets {
            ...
            rows: vec![vec![Binding {
                name: "doc".into(),
                value: string_literal("math_const_tau() -> the full circle constant (2*pi)."),
            }]],
        }
    }
}
