//! math_const_tau — mathematical constant tau.
//!
//! Ported from the module-mode ABI (semantalytics/stardog-webfunctions/
//! function_math_constants/tau) to the Component Model ABI.
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "math_const_tau" => evaluate_impl(args),
            other => Err(format!("math_const_tau: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
        if !args.is_empty() {
            return Err(format!("math_const_tau: expected 0 args, got {}", args.len()));
        }
        Ok(decimal_literal(consts::TAU))
    }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "math_const_tau: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("math_const_tau: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "math_const_tau: unknown property function '{name}' (this component provides none)"
        ))
    }
}
