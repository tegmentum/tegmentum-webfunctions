...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, _args: Vec<WitTerm>) -> Result<WitTerm, String> {
        Err(format!(
            "string_lang_detect_confidence: unknown filter function '{name}' (use as a property function)"
        ))
    }
}
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "string_lang_detect_confidence: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("string_lang_detect_confidence: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    fn register_property_functions() -> Vec<PropertyDescriptor> {
        vec![PropertyDescriptor {
            name: "string_lang_detect_confidence".to_string(),
            subject_arity: 0,
            object_arity: 0,
        }]
    }
    ...
    fn evaluate(
        ...
        match name.as_str() {
            "string_lang_detect_confidence" => {
                let mut args = subjects;
                args.extend(objects);
                let bs = evaluate_impl(args)?;
                Ok(to_binding_rows(bs))
            }
            other => Err(format!("string_lang_detect_confidence: unknown property function '{other}'")),
        }
    }
}
