...
impl Guest for Component {
    fn evaluate(args: Vec<Value>) -> Result<BindingSets, String> {
        ...
        Ok(BindingSets { vars: vec!["lang".into()], rows: vec![vec![
            Binding { name: "lang".into(), value: string_literal(lang.iso_code_639_1().to_string().to_lowercase().as_str()) }]] })
    }
    ...
    fn doc() -> BindingSets { BindingSets { vars: vec!["doc".into()], rows: vec![vec![Binding {
        name: "doc".into(), value: string_literal("string_lang_lingua_detect(text) -> ISO 639-1 code, via lingua-rs.")}]] } }
}
export!(Component);
