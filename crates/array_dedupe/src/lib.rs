//! array_dedupe — remove *consecutive* duplicate elements from an array
//! literal (`Vec::dedup` semantics — like Unix `uniq`, not full de-dup).
//! Use `array_unique` if you want all duplicates removed.
//!
//! Ports semantalytics/stardog-webfunctions/function_array/dedupe.
    ...
    match v {
        WitTerm::Literal(l) if l.datatype.as_deref().unwrap_or("http://www.w3.org/2001/XMLSchema#string") == ARRAY_DATATYPE => {
            let parsed: JsonValue = serde_json::from_str(&l.value)
                .map_err(|e| format!("array_dedupe: invalid array literal JSON: {}", e))?;
            match parsed {
                JsonValue::Array(a) => Ok(a),
                _ => Err("array_dedupe: array literal is not a JSON array".into()),
            }
        }
        _ => Err(format!("array_dedupe: expected array literal (datatype {})", ARRAY_DATATYPE)),
    }
}
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "array_dedupe" => evaluate_impl(args),
            other => Err(format!("array_dedupe: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err(format!("array_dedupe: expected 1 arg, got {}", args.len()));
        }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "array_dedupe: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("array_dedupe: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "array_dedupe: unknown property function '{name}' (this component provides none)"
        ))
    }
}
