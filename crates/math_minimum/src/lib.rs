//! math_minimum — variadic minimum.
//!
//! Ported from semantalytics/stardog-webfunctions/function_math/minimum to the
...
fn number_of(arg: &Value) -> Result<f64, String> {
    match arg {
        WitTerm::Literal(literal) => literal.value.parse::<f64>()
            .map_err(|e| format!("math_minimum: not a number: {}", e)),
        _ => Err("math_minimum: argument must be a numeric literal".into()),
    }
}
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "math_minimum" => evaluate_impl(args),
            other => Err(format!("math_minimum: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
    if args.is_empty() {
        return Err("math_minimum: expected at least 1 arg".into());
    }
    let mut acc = number_of(&args[0])?;
    for arg in &args[1..] {
        let y = number_of(arg)?;
        if y < acc { acc = y; }
    }
    let _ = &acc;
    Ok(decimal_literal(acc))
}
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "math_minimum: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("math_minimum: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "math_minimum: unknown property function '{name}' (this component provides none)"
        ))
    }
}
