//! math_square_root — square root.
//!
//! Ported from semantalytics/stardog-webfunctions/function_math/square_root to the
...
fn number_of(arg: &Value) -> Result<f64, String> {
    match arg {
        WitTerm::Literal(literal) => literal.value.parse::<f64>()
            .map_err(|e| format!("math_square_root: not a number: {}", e)),
        _ => Err("math_square_root: argument must be a numeric literal".into()),
    }
}
...
impl ExtensionGuest for Component {
    fn register() -> Vec<FunctionDescriptor> {
        vec![FunctionDescriptor {
            name: "math_square_root".to_string(),
            min_arity: 0,
            max_arity: None,
        }]
    }
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "math_square_root" => evaluate_impl(args),
            other => Err(format!("math_square_root: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err(format!("math_square_root: expected 1 arg, got {}", args.len()));
    }
    let x = number_of(&args[0])?;
    if x < 0.0 { return Err(format!("math_square_root: negative input {}", x)); }
    let y = x.sqrt();
    if !y.is_finite() {
        return Err(format!("math_square_root: non-finite result {}", y));
    }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "math_square_root: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("math_square_root: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "math_square_root: unknown property function '{name}' (this component provides none)"
        ))
    }
}
