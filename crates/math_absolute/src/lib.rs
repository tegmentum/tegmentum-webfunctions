...
//!
//! Ported from semantalytics/stardog-webfunctions/function_math/absolute to the
...
fn number_of(arg: &Value) -> Result<f64, String> {
    match arg {
        WitTerm::Literal(literal) => literal.value.parse::<f64>()
            .map_err(|e| format!("math_absolute: not a number: {}", e)),
        _ => Err("math_absolute: argument must be a numeric literal".into()),
    }
}
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "math_absolute" => evaluate_impl(args),
            other => Err(format!("math_absolute: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err(format!("math_absolute: expected 1 arg, got {}", args.len()));
    }
    let x = number_of(&args[0])?;
    let y = x.abs();
    if !y.is_finite() {
        return Err(format!("math_absolute: non-finite result {}", y));
    }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "math_absolute: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("math_absolute: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "math_absolute: unknown property function '{name}' (this component provides none)"
        ))
    }
}
