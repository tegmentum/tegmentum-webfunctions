...
pub fn sweep(
    ...
    if target_sink_url.is_empty() {
        return Err("wf_sagegraph: sweep target-sink-url must be non-empty".into());
    }
    let dimensions = opts.dimensions;
    if dimensions == 0 {
        return Err("wf_sagegraph: sweep opts.dimensions must be >= 1".into());
    }
    ...
    for s in subjects {
        ...
        let body = serde_json::to_string(&row)
            .map_err(|e| format!("wf_sagegraph: encode sweep row: {e}"))?;
        ...
        host.http_post_json(target_sink_url, &body)
            .map_err(|e| format!("wf_sagegraph: sink POST failed for {s}: {e}"))?;
        written += 1;
    }
    Ok(written)
}
...
fn enumerate_subjects(
    ...
    let parsed: crate::embed_kernel::SparqlResults = serde_json::from_str(&json)
        .map_err(|e| format!("wf_sagegraph: parse subject SPARQL Results: {e}"))?;
    let mut out = Vec::new();
