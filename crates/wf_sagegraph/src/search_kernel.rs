...
pub fn search(
    ...
    if index_url.is_empty() {
        return Err("wf_sagegraph: search index-url must be non-empty".into());
    }
    if k == 0 {
        return Err("wf_sagegraph: search k must be >= 1".into());
    }
    ...
    let body_str = serde_json::to_string(&body)
        .map_err(|e| format!("wf_sagegraph: encode search body: {e}"))?;
    let resp = host.http_post_json(index_url, &body_str)?;
    let parsed: SearchResponse = serde_json::from_str(&resp)
        .map_err(|e| format!("wf_sagegraph: parse vector-index response: {e}"))?;
    Ok(parsed
        .hits
        .into_iter()
        .map(|h| Hit {
