//! string_lang_lingua_all_languages — enumerate the languages compiled into this crate.
//!
//! Rows: one ISO 639-1 code per language.
...
fn evaluate_impl(_args: Vec<Value>) -> Result<BindingSets, String> {
        let langs: Vec<Language> = all_langs().into_iter().filter(|l| true).collect();
        let rows = langs.into_iter().map(|l| vec![Binding { name: "lang".into(),
            value: string_literal(l.iso_code_639_1().to_string().to_lowercase().as_str()) }]).collect();
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, _args: Vec<WitTerm>) -> Result<WitTerm, String> {
        Err(format!(
            "string_lang_lingua_all_languages: unknown filter function '{name}' (use as a property function)"
        ))
    }
}
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "string_lang_lingua_all_languages: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("string_lang_lingua_all_languages: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    fn register_property_functions() -> Vec<PropertyDescriptor> {
        vec![PropertyDescriptor {
            name: "string_lang_lingua_all_languages".to_string(),
            subject_arity: 0,
            object_arity: 0,
        }]
    }
    ...
    fn evaluate(
        ...
        match name.as_str() {
            "string_lang_lingua_all_languages" => {
                let mut args = subjects;
                args.extend(objects);
                let bs = evaluate_impl(args)?;
                Ok(to_binding_rows(bs))
            }
            other => Err(format!("string_lang_lingua_all_languages: unknown property function '{other}'")),
        }
    }
}
