//! math_plus — addition.
//!
//! Ported from semantalytics/stardog-webfunctions/function_math/plus to the
...
fn number_of(arg: &Value) -> Result<f64, String> {
    match arg {
        WitTerm::Literal(literal) => literal.value.parse::<f64>()
            .map_err(|e| format!("math_plus: not a number: {}", e)),
        _ => Err("math_plus: argument must be a numeric literal".into()),
    }
}
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "math_plus" => evaluate_impl(args),
            other => Err(format!("math_plus: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 2 {
        return Err(format!("math_plus: expected 2 args, got {}", args.len()));
    }
    let a = number_of(&args[0])?;
    let b = number_of(&args[1])?;
    let y = a + b;
    if !y.is_finite() {
        return Err(format!("math_plus: non-finite result {}", y));
    }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "math_plus: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("math_plus: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "math_plus: unknown property function '{name}' (this component provides none)"
        ))
    }
}
