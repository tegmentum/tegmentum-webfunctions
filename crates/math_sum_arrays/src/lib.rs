...
fn evaluate(args: Vec<Value>) -> Result<BindingSets, String> {
    ...
    Ok(BindingSets {
        ...
        rows: vec![vec![Binding { name: "result".into(), value: decimal_literal(acc) }]],
    })
}

    fn aggregate_step(_args: Vec<Value>, _mult: u64) -> Result<(), String> {
        Err("math_sum_arrays: aggregate not applicable".into())
    }
    ...
    fn doc() -> BindingSets {
        BindingSets {
            ...
            rows: vec![vec![Binding {
                name: "doc".into(),
                value: string_literal("math_sum_arrays(x1, x2, ...) -> x1 + x2 + ... The upstream source was empty; this treats the arg list as a flat sequence to sum. Empty list returns 0."),
            }]],
        }
    }
}
