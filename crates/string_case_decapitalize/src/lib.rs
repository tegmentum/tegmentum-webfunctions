//! string_case_decapitalize — lowercase the first character, optionally
//! lowercasing the rest.
//!
//! Wraps voca_rs::case::decapitalize(subject, rest_to_lower). Accepts either
...
fn string_of(a: &Value) -> Result<&str, String> {
    match a {
        ...
        _ => Err("string_case_decapitalize: arguments must be literals".into()),
    }
}
...
impl ExtensionGuest for Component {
    fn register() -> Vec<FunctionDescriptor> {
        vec![FunctionDescriptor {
            name: "string_case_decapitalize".to_string(),
            min_arity: 0,
            max_arity: None,
        }]
    }
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "string_case_decapitalize" => evaluate_impl(args),
            other => Err(format!("string_case_decapitalize: unknown function '{other}'")),
        }
    }
}
        ...
        let (subject, rest_to_lower) = match args.len() {
            ...
            2 => {
                ...
                let flag = string_of(&args[1])?
                    .parse::<bool>()
                    .map_err(|e| format!("string_case_decapitalize: bad bool arg: {}", e))?;
                (s, flag)
            }
            n => return Err(format!("string_case_decapitalize: expected 1 or 2 args, got {}", n)),
        };
        let out = voca_rs::case::decapitalize(subject, rest_to_lower);
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "string_case_decapitalize: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("string_case_decapitalize: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "string_case_decapitalize: unknown property function '{name}' (this component provides none)"
        ))
    }
}
