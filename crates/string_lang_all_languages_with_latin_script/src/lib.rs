//! string_lang_all_languages_with_latin_script — enumerate whatlang languages canonically written in the Latin script.
//!
...
const SCRIPT_LANGS: &[&str] = &["epo", "eng", "por", "spa", "aka", "zul", "yor", "ind", "jav", "xho", "afr", "hrv", "ces", "dan", "est", "fin", "fra", "deu", "hun", "ita", "lat", "lav", "lit", "mri", "nld", "nob", "pol", "ron", "slk", "slv", "sme", "swe", "tuk", "tur", "uzb", "vie", "aze", "ilo", "tgl", "cat"];
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, _args: Vec<WitTerm>) -> Result<WitTerm, String> {
        Err(format!(
            "string_lang_all_languages_with_latin_script: unknown filter function '{name}' (use as a property function)"
        ))
    }
}
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "string_lang_all_languages_with_latin_script: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("string_lang_all_languages_with_latin_script: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    fn register_property_functions() -> Vec<PropertyDescriptor> {
        vec![PropertyDescriptor {
            name: "string_lang_all_languages_with_latin_script".to_string(),
            subject_arity: 0,
            object_arity: 0,
        }]
    }
    ...
    fn evaluate(
        ...
        match name.as_str() {
            "string_lang_all_languages_with_latin_script" => {
                let mut args = subjects;
                args.extend(objects);
                let bs = evaluate_impl(args)?;
                Ok(to_binding_rows(bs))
            }
            other => Err(format!("string_lang_all_languages_with_latin_script: unknown property function '{other}'")),
        }
    }
}
