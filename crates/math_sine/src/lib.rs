//! math_sine — sine (radians).
//!
//! Ported from semantalytics/stardog-webfunctions/function_math/sine to the
...
fn number_of(arg: &Value) -> Result<f64, String> {
    match arg {
        WitTerm::Literal(literal) => literal.value.parse::<f64>()
            .map_err(|e| format!("math_sine: not a number: {}", e)),
        _ => Err("math_sine: argument must be a numeric literal".into()),
    }
}
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "math_sine" => evaluate_impl(args),
            other => Err(format!("math_sine: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
    if args.len() != 1 {
        return Err(format!("math_sine: expected 1 arg, got {}", args.len()));
    }
    let x = number_of(&args[0])?;
    let y = x.sin();
    if !y.is_finite() {
        return Err(format!("math_sine: non-finite result {}", y));
    }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "math_sine: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("math_sine: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "math_sine: unknown property function '{name}' (this component provides none)"
        ))
    }
}
