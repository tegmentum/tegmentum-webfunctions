//! hash_blake2b — hex-encoded BLAKE2b-512 of a UTF-8 string literal.
...
fn string_of(arg: &Value) -> Result<&str, String> {
    match arg {
        ...
        _ => Err("hash_blake2b: argument must be a string literal".into()),
    }
}
...
impl ExtensionGuest for Component {
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "hash_blake2b" => evaluate_impl(args),
            other => Err(format!("hash_blake2b: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err(format!("hash_blake2b: expected 1 arg, got {}", args.len()));
        }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "hash_blake2b: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("hash_blake2b: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "hash_blake2b: unknown property function '{name}' (this component provides none)"
        ))
    }
}
