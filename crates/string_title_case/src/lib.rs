//! string_title_case — convert to Title Case (space-separated).
//!
...
fn string_of(a: &Value) -> Result<&str, String> {
    match a {
        ...
        _ => Err("string_title_case: argument must be a string literal".into()),
    }
}
...
impl ExtensionGuest for Component {
    fn register() -> Vec<FunctionDescriptor> {
        vec![FunctionDescriptor {
            name: "string_title_case".to_string(),
            min_arity: 0,
            max_arity: None,
        }]
    }
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "string_title_case" => evaluate_impl(args),
            other => Err(format!("string_title_case: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err(format!("string_title_case: expected 1 arg, got {}", args.len()));
        }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "string_title_case: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("string_title_case: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "string_title_case: unknown property function '{name}' (this component provides none)"
        ))
    }
}
