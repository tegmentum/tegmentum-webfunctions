//! string_case_lower_first — lowercase the first character.
//!
//! Wraps voca_rs::case::lower_first.
...
fn string_of(a: &Value) -> Result<&str, String> {
    match a {
        ...
        _ => Err("string_case_lower_first: argument must be a string literal".into()),
    }
}
...
impl ExtensionGuest for Component {
    fn register() -> Vec<FunctionDescriptor> {
        vec![FunctionDescriptor {
            name: "string_case_lower_first".to_string(),
            min_arity: 0,
            max_arity: None,
        }]
    }
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "string_case_lower_first" => evaluate_impl(args),
            other => Err(format!("string_case_lower_first: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
        if args.len() != 1 {
            return Err(format!("string_case_lower_first: expected 1 arg, got {}", args.len()));
        }
        let out = voca_rs::case::lower_first(string_of(&args[0])?);
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "string_case_lower_first: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("string_case_lower_first: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "string_case_lower_first: unknown property function '{name}' (this component provides none)"
        ))
    }
}
