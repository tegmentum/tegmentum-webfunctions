//! math_const_frac_pi_4 — mathematical constant pi/4.
//!
//! Ported from the module-mode ABI (semantalytics/stardog-webfunctions/
//! function_math_constants/frac_pi_4) to the Component Model ABI.
...
impl ExtensionGuest for Component {
    fn register() -> Vec<FunctionDescriptor> {
        vec![FunctionDescriptor {
            name: "math_const_frac_pi_4".to_string(),
            min_arity: 0,
            max_arity: None,
        }]
    }
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "math_const_frac_pi_4" => evaluate_impl(args),
            other => Err(format!("math_const_frac_pi_4: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
        if !args.is_empty() {
            return Err(format!("math_const_frac_pi_4: expected 0 args, got {}", args.len()));
        }
        Ok(decimal_literal(consts::FRAC_PI_4))
    }
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "math_const_frac_pi_4: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("math_const_frac_pi_4: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "math_const_frac_pi_4: unknown property function '{name}' (this component provides none)"
        ))
    }
}
