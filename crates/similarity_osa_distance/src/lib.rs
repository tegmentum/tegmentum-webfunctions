//! similarity_osa_distance — Optimal String Alignment distance between two strings.
//!
//! Ports the semantalytics function_string_similarity/osa_distance crate.
...
fn string_of(arg: &Value) -> Result<&str, String> {
    match arg {
        ...
        _ => Err("similarity_osa_distance: arguments must be string literals".into()),
    }
}
...
impl ExtensionGuest for Component {
    fn register() -> Vec<FunctionDescriptor> {
        vec![FunctionDescriptor {
            name: "similarity_osa_distance".to_string(),
            min_arity: 0,
            max_arity: None,
        }]
    }
    ...
    fn call(name: String, args: Vec<WitTerm>) -> Result<WitTerm, String> {
        match name.as_str() {
            "similarity_osa_distance" => evaluate_impl(args),
            other => Err(format!("similarity_osa_distance: unknown function '{other}'")),
        }
    }
}
...
fn evaluate_impl(args: Vec<Value>) -> Result<Value, String> {
        if args.len() != 2 {
            return Err(format!("similarity_osa_distance: expected 2 args, got {}", args.len()));
        }
        let a = string_of(&args[0])?;
        let b = string_of(&args[1])?;
        let d = strsim::osa_distance(a, b) as i64;
...
impl AggregateGuest for Component {
    ...
    fn new_aggregate(name: String) -> Result<AggregateState, String> {
        Err(format!(
            "similarity_osa_distance: unknown aggregate '{name}' (this component provides none)"
        ))
    }
}
...
impl GuestAggregateState for UnreachableState {
    fn step(&self, _args: Vec<WitTerm>) -> Result<(), String> {
        Err("similarity_osa_distance: aggregate state was never constructed".into())
    }
...
impl PropertyFunctionGuest for Component {
    ...
    fn evaluate(
        ...
        Err(format!(
            "similarity_osa_distance: unknown property function '{name}' (this component provides none)"
        ))
    }
}
